...
document.addEventListener('click', function(event) {
    ...
    if (event.target.classList.contains('submit-btn')) {
        ...
        // 1. Find the parent container for this specific item
        const parent = event.target.closest('.item-container');
        
        // 2. Find the editor inside that parent
        const editorDiv = parent.querySelector('.editor');
        
        // 3. Pull the Quill instance we stored earlier
        ...
        if (quill) {
            ...
            // Now you can send htmlContent to your database/API
...
function show_hide_combat_modifiers(select_element) {
    ...
    if (item_type_input.value == 'weapon' || item_type_input.value == 'armor' || item_type_input.value == 'shield') {
...
function fill_subtypes(select_element) {
    ...
    if (item_type.value == 'weapon') {
        ...
        subtype_select.innerHTML = `
        <label>Subtype: </label>
        <select class="dropdown" id="subtype-select">
...
function update_ui(adversary) {
    ...
    adversary.inventory.forEach(item => {
        const item_block = `
            <div id="${item.id}" class="equipment-item bg-slate-600 rounded-md p-3 mr-4 mb-4 max-w-sm">
